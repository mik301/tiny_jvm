...
public final class VM {
    ...
    private void invokeSpecial(Frame f, LoadedPackage pkg, int cpIndex) {
        ...
        if (ac != null) {
            ...
            if (key == null) {
                throw new LinkException("no API binding for static token " + e.token
                        + " on " + ac.name);
            }
    ...
    private void invokeNative(Frame caller, String key, boolean consumesSelf) {
        ...
        if (impl == null) {
            throw new LinkException("API method not implemented by this VM: " + key);
        }
    ...
    public String apiVirtualKey(String className, int token) {
        ...
        if (key == null) {
            throw new LinkException("no token table entry for virtual token " + token
                    + " of " + className + " - check api-tokens.txt");
        }
        return key;
    }
